...
function M.progress(data)
    if not data or not data.result then
        return -- Prevents crash when result is nil
    end

    local client_id = data.client_id
    local result = data.result
    local id = client_id .. "." .. tostring(result.token) -- Ensure token is safely converted to string
    ...
    if not message then
        ...
        message.opts.progress = {
            client_id = client_id,
            ---@type string
            client = client.name or ("lsp-" .. client_id),
        }
        M._progress[id] = message
    end

    -- Ensure result.value is valid before accessing it
